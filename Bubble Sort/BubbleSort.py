...
for passs in range(passes) : # loopoing over the passes
    ...
    for i in range(len(List) - 1) : # looping through the list of numbers
        ...
        comparisons += 1
        if (a > b) : 
            SwapElements(List, i, i+1) # swaps the elements on that iteration index
            swaps += 1 
            ColorPrint(List, i, Fore.RED) # prints the numbers of this comparision color coded with red in the list
        else :
            ColorPrint(List, i, Fore.GREEN) # prints the numbers of this comparision color coded with green in the list
